...
#define LCD_ADD ((LCD_TypeDef *) LCD_BASE)
/**
  * @}
  */ 
...
/* Configure the FSMC Parallel interface -------------------------------------*/
  ...
  else 
  {
    ...
    LCD_RD_DATA();//IC版本
    lcdid = LCD_RD_DATA();
    lcdid = lcdid<<8|LCD_RD_DATA();
    
        if(lcdid==0x9341)
    {
      LCD_WR_REG(0xCF);//电源控制B
      LCD_WriteRAM(0x00); 
      LCD_WriteRAM(0xC1); 
      LCD_WriteRAM(0X30); 
      
      LCD_WR_REG(0xED);//上电顺序控制
      LCD_WriteRAM(0x64); 
      LCD_WriteRAM(0x03); 
      LCD_WriteRAM(0X12); 
      LCD_WriteRAM(0X81); 
      ...
      LCD_WriteRAM(0x01);   //SAP[2:0];BT[3:0] 
      
      LCD_WR_REG(0xC5);//VCOM Control 1
      
      
      /////////////////////这里改VCOM电压，否则显示会灰蒙蒙的感觉
      LCD_WriteRAM(0x5C); 	 //3F
      LCD_WriteRAM(0x0C); 	 //3C
      
      LCD_WR_REG(0xC7);//VCOM Control 2
      LCD_WriteRAM(0x8F); 
      
      LCD_WR_REG(0x36);//内存接入控制
      LCD_WriteRAM(0x48);
      ...
      LCD_WR_REG(0xF2);    //使能3Gamma
      LCD_WriteRAM(0x00); 
      
      LCD_WR_REG(0x26);    //Gamma设置
      LCD_WriteRAM(0x01); 
      
      LCD_WR_REG(0xE0);    //正Gamma修正
      LCD_WriteRAM(0x0F); 
      LCD_WriteRAM(0x20); 
      LCD_WriteRAM(0x19); 
      LCD_WriteRAM(0x0F); 
...
//  LCD_WriteReg(LCD_REG_33, Ypos);
    LCD_WR_REG(0X2A); 
 // LCD_WriteRAM(Xpos>>8);LCD_WriteRAM(Xpos&0XFF); 	
  LCD_WriteRAM(Xpos>>8);LCD_WriteRAM(Xpos&0XFF); 	
  LCD_WR_REG(0X2B); 
 // LCD_WriteRAM(Ypos>>8);LCD_WriteRAM(Ypos&0XFF); 	
  LCD_WriteRAM(Ypos>>8);LCD_WriteRAM(Ypos&0XFF); 
}
