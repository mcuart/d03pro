...
/*************************定义最小轮询时间，例程里默认的10时间太长会导致usb扫码枪输入漏读，
每次只能读到第一个数字，改成5就可以全部读完
UCOS任务中必须以信号量方式查询执行，OSTaskSemPend(0,OS_OPT_PEND_BLOCKING,0,&err);//请求任务内建的信号量
在OS_TickTask任务中发送OSTaskSemPost信号量
*********************************/
//#define HID_MIN_POLL          10 
#define HID_MIN_POLL          6
...
{
  void  (*Init)   (void);             
  void  (*Decode) (uint8_t *data, uint8_t *data_shift);       
