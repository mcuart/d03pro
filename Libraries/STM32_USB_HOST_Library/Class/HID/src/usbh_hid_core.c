                                   ...
                                   void   *phost)
{
  ...
  static u8 key_code_index[64];
  static u8 key_code_shift[64];
  static u8 index=0;
  ...
  case HID_POLL:
   ...
   // HID_Machine.cb->Decode(HID_Machine.buff);
        ...
        if(HID_Machine.buff[2]!=0)//键盘按键弹起为0
        {
          if(index==0)
          {
            memset(key_code_index,0,64);
          }

          key_code_index[index]=HID_Machine.buff[2];
          key_code_shift[index]=HID_Machine.buff[0];
          index++;
          if(key_code_index[index-1]==0x28)//KB_ENTER 20250515遇到回车解码一次
          {
//            if(index!=15)
//            {
//              index=index;
...
//            cp++;
            ...
            HID_Machine.cb->Decode(key_code_index,key_code_shift);
