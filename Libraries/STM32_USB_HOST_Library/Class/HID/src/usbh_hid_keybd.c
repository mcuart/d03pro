...
static void  KEYBRD_Decode(uint8_t *data,u8 *pbuf_shift);
...
*******************************************************************************
*/
//键位表。数组下标数值对应实际HID键盘输入的8字节数据的第[2]字节数值
...
static void KEYBRD_Decode(u8 *pbuf,u8 *pbuf_shift)
{
  u8 ix=0;
  static u8 key_arry[64];
  static uint8_t shift;


  
  
  while(*(pbuf+ix)!=0x28)//KB_ENTER CODE
  {
    /* Check if Shift key is pressed */                                                                         
    if ((*(pbuf_shift+ix) == KBD_LEFT_SHIFT) || (*(pbuf_shift+ix) == KBD_RIGHT_SHIFT)) {
      shift = TRUE;
    } else {
      shift = FALSE;
    } 
    ...
    if (shift == TRUE) {
      key_arry[ix] =  HID_KEYBRD_ShiftKey[HID_KEYBRD_Codes[*(pbuf+ix)]];
    } else {
      key_arry[ix] =  HID_KEYBRD_Key[HID_KEYBRD_Codes[*(pbuf+ix)]];
    }
    ix++;
  }
  ...
  /* call user process handle */
  USR_KEYBRD_ProcessData(key_arry);
