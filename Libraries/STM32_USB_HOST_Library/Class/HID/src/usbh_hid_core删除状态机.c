...
//    break;
  ...
  case HID_POLL:
    poll_time=HCD_GetCurrentFrame(pdev) - HID_Machine.timer;
    if(poll_time >= HID_Machine.poll)
    {
      //HID_Machine.state = HID_GET_DATA;
    ...
    if(HCD_GetURB_State(pdev , HID_Machine.hc_num_in) == URB_DONE)
    {
      if(start_toggle == 1) /* handle data once */
      {
        ...
        if(HID_Machine.buff[2]!=0)
        {
          ...
          index++;
          if(key_code_index[index-1]==0x28)//KB_ENTER
          {

            HID_Machine.cb->Decode(key_code_index);
            index=0;
          }
        }
