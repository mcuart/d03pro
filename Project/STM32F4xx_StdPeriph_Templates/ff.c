...
)
{
	...
	return ncl;		/* Menu new cluster number */
}
#endif /* !_FS_READONLY */
...
)
{
	...
	if (vol <= 9 && p[1] == ':') {	/* Found a drive number, get and strip it */
		p += 2; *path = p;			/* Menu pointer to the path name */
...
/*-----------------------------------------------------------------------*/
/* Change Timestamp   文件创建时间                                       */
/*-----------------------------------------------------------------------*/

FRESULT f_utime_create (
	const XCHAR *path,	/* Pointer to the file/directory name */
	const FILINFO *fno	/* Pointer to the timestamp to be set */
)
{
		...
		if (res == FR_OK) {
			...
			if (!dir) {				/* Root directory */
				res = FR_INVALID_NAME;
                                ...
                                ST_WORD(dir+DIR_CrtTime, fno->ftime);//文件创建时间  
                                ST_WORD(dir+DIR_CrtDate, fno->fdate);//文件创建时间  
				dj.fs->wflag = 1;
				res = sync(dj.fs);
			}
		}
...
)
{
	...
	return bw ? chr : EOF;		/* Menu the result */
}
