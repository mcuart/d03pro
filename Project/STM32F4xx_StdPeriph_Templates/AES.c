...
void AES_Test_Encrypt(u8 *dat,u32 data_length,u32 push_id)
{
	...
	u8 chainCipherBlock[16];
    unsigned char i;
	for(i=0;i<32;i++) AES_Key_Table[i]=i;//做运算之前先要设置好密钥，这里只是设置密钥的DEMO。
	AES_Key_Table[0]= (u8)push_id;
	AES_Key_Table[1] = (u8)(push_id>>8);
	AES_Key_Table[2] = (u8)(push_id>>16);
	AES_Key_Table[3] = (u8)(push_id>>24);
 



	memset(chainCipherBlock,0x00, data_length);
...
void AES_Test_Decrypt(u8 *dat, u32 data_length, u32 push_id)
{
	...
	memset(chainCipherBlock, 0x00, data_length);//这里要重新初始化清空
...
{
  CopyBytes( temp, dat, BLOCKSIZE );
  InvCipher( dat, expandedKey );
  XORBytes( dat, chainCipherBlock, BLOCKSIZE );
  CopyBytes( chainCipherBlock, temp, BLOCKSIZE );
}
...
{
  XORBytes(dat, chainCipherBlock, BLOCKSIZE);
  Cipher(dat, expandedKey);
  CopyBytes(chainCipherBlock, dat, BLOCKSIZE);
}
