...
// Functions
extern void		SWJ_Sequence	(SWD_HANDLE *pswd, uint32_t count, uint8_t *data);
...
extern uint8_t	SWD_Transfer	(SWD_HANDLE *pswd, uint8_t request, uint32_t *data);

extern void		Delayms			(uint32_t delay);

extern uint32_t	DAP_ProcessVendorCommand(uint8_t *request, uint8_t *response);

extern uint32_t	DAP_ProcessCommand(SWD_HANDLE *pswd, uint8_t *request, uint8_t *response);
